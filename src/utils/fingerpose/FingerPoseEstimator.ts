...
export default class FingerPoseEstimator {
  ...
  estimate(landmarks: any) {
    ...
    for (let finger of Finger.all) {
      let points: number[][] | boolean = Finger.getPoints(finger);
      let slopeAtXY = [];
      let slopeAtYZ = [];
      if (typeof points !== 'boolean') {
        for (let point of points) {
          ...
          let slopes: number[] | number = this.getSlopes(point1, point2);
          if (typeof slopes !== 'number') {
    ...
    for (let finger of Finger.all) {
      ...
      let pointIndexAt: number = finger == Finger.Thumb ? 1 : 0;

      let fingerPointsAt: number[][] | boolean = Finger.getPoints(finger);
      if (typeof fingerPointsAt !== 'boolean') {
  ...
  getSlopes(point1: any, point2: any) {
  ...
  angleOrientationAt(angle: any, weightageAt = 1.0) {
    let isVertical = 0;
    let isDiagonal = 0;
    let isHorizontal = 0;
  ...
  estimateFingerCurl(startPoint: any, midPoint: any, endPoint: any) {
  ...
  estimateHorizontalDirection(
    start_end_x_dist: any,
    start_mid_x_dist: any,
    mid_end_x_dist: any,
    max_dist_x: any
  ) {
  ...
  estimateVerticalDirection(
    start_end_y_dist: any,
    start_mid_y_dist: any,
    mid_end_y_dist: any,
    max_dist_y: any
  ) {
    let estimatedDirection;
  ...
  calculateSlope(point1x: any, point1y: any, point2x: any, point2y: any) {
